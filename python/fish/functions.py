...
def generateSetFish(indices, goOvers, training=False):
    setSize = len(indices)*len(np.atleast_1d(s.miniSets))*goOvers
    dataset_images = np.zeros((setSize, 2, s.imageSize, s.imageSize), dtype=int)
    dataset_labels = []
    centers_1 = (0, 0, (1184, 445), (1096, 386), (950, 407), (870, 297), (1006, 249), (1128, 328))
    centers_2 = (0, 0, (923, 446), (1075, 505), (1182, 480), (1235, 324), (1137, 313), (975, 309))
    lengths_1 = (0, 0, 303, 273, 254, 280, 301, 314)
    lengths_2 = (0, 0, 262, 301, 274, 221, 300, 312)
    bar = progressbar.ProgressBar(maxval=setSize, widgets=[progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage()])
    bar.start()
    b = 0
    for j in range(goOvers):
        for m in np.atleast_1d(s.miniSets):
            # Get corresponding labels for dataset
            labelName = 'm{0}.txt'.format(m)
            labelPath = os.path.join(s.labelFolder, labelName)
            new_labels = np.loadtxt(labelPath, dtype='float', delimiter=',')
            new_labels = new_labels[indices, :]
            dataset_labels.append(new_labels)

            for i in indices:
                bar.update(b+1)

                # Select image
                nameOfImage_1 = 'm{0}/img_1/img_{1}.png'.format(m, i+1)
                nameOfImage_2 = 'm{0}/img_2/img_{1}.png'.format(m, i+1)
                imagePath_1 = os.path.join(s.pathOfImages, nameOfImage_1)
                imagePath_2 = os.path.join(s.pathOfImages, nameOfImage_2)
                ...
                # Crop and rotate images randomly
                img_1 = cropImage(img_1, centers_1[m], lengths_1[m])
                img_2 = cropImage(img_2, centers_2[m], lengths_2[m])

                # Process grayscale image to binary image
                img_1 = processImage(np.uint8(img_1))
                img_2 = processImage(np.uint8(img_2))

                dataset_images[b,0,:,:] = img_1/255
                dataset_images[b,1,:,:] = img_2/255
                
                b += 1

    dataset_labels = np.vstack(dataset_labels)

    # Normalize labels
    if training:
        labels_mean = np.mean(dataset_labels, axis=0)
        labels_std = np.std(dataset_labels, axis=0)
        dataset_labels = (dataset_labels - labels_mean) / labels_std
        np.savetxt('../fish_data/labels_mean.txt', labels_mean, delimiter=',')
        np.savetxt('../fish_data/labels_std.txt', labels_std, delimiter=',')
    else:
        labels_mean = np.loadtxt('../fish_data/labels_mean.txt', dtype='float', delimiter=',')
        labels_std = np.loadtxt('../fish_data/labels_std.txt', dtype='float', delimiter=',')
        dataset_labels = (dataset_labels - labels_mean) / labels_std

    # dataset_images = torch.FloatTensor(dataset_images).to(s.device)
    # dataset_labels = torch.FloatTensor(dataset_labels).to(s.device)
...
def generateBatchesFish(dataset_img, dataset_lab, batchSize):
    dataset = TensorDataset(dataset_img, dataset_lab)
    batches = DataLoader(dataset, batch_size=batchSize, shuffle=True)
    return batches
