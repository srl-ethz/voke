...
def main():
    ...
    # Import dataset
    ###################################################################################################
    # Locate the center of the arm in order to crop the image later
    # print("Define center of arm...")
    # calibrationImagePath_1 = os.path.join(args.data, args.calibration_image_1)
    # calibrationImagePath_2 = os.path.join(args.data, args.calibration_image_2)
    # armCenter_1, armLength_1 = f.findCenter(calibrationImagePath_1)
    # armCenter_2, armLength_2 = f.findCenter(calibrationImagePath_2)
    # armCenter_1 = np.array([520.5, 1060.])
    # armCenter_2 = np.array([559.5, 999.])
    # armLength_1 = 750
    # armLength_2 = 730
    ...
    if os.path.isfile(trainset_path):
        ...
        trainingSet = f.generateSet(trainingSetIndices, label_idx, args.data)
    ...
    if os.path.isfile(validset_path):
        ...
        validationSet = f.generateSet(validationSetIndices, label_idx, args.data)
    ...
    testset_path = os.path.join(datasetPath, 'testingSet.pkl')
    testset_indice_path = os.path.join(datasetPath, 'testingSetIndices.txt')
    if os.path.isfile(testset_path):
        print("Loading Testig Set...")
        with open(testset_path, 'rb') as handle:
            testingSet = pickle.load(handle)
        testingSetIndices = np.loadtxt(testset_indice_path, dtype='float')
        testingSetIndices = [int(x) for x in testingSetIndices]

    else:
        print("\nCreating Testing Set...")
        testingSet = f.generateSet(testingSetIndices, label_idx, args.data)
        np.savetxt(testset_indice_path, testingSetIndices)
        with open(testset_path, 'wb') as handle:
    ...
    for epoch in range(args.epochs):
        ...
        # Create new batches
        trainingBatches = f.generateBatches(trainingSet, trainingSetIndices, args.batch_size, args.training_go_overs, label)
        validationBatches = f.generateBatches(validationSet, validationSetIndices, args.batch_size, args.validation_go_overs, label)
